...
| `create_character` | bool | 从生成的视频创建角色（用于图片生成视频场景），默认 `false` |
...
## 功能七：图片生成视频并创建角色（一步完成）

上传参考图生成视频，并自动从生成的视频中提取角色。适用于只有图片素材但想创建角色的场景。
  ...
  -d '{
    ...
    "messages": [{
      ...
      "content": [
        ...
        {
          "type": "text",
          "text": "人物开始微笑，然后慢慢转头看向镜头"
        }
      ]
    }],
    "create_character": true,
    "character_description": "一位面带微笑的人物",
    "stream": true
  }'
```
...
# 读取图片
with open("character_photo.jpg", "rb") as f:
    image_base64 = base64.b64encode(f.read()).decode()
...
response = requests.post(
    ...
    json={
        ...
        "create_character": True,  # 关键参数：从生成的视频创建角色
        "character_description": "一位面带微笑的人物",  # 可选
        "stream": True
    },
    stream=True,
    timeout=900  # 建议设置较长超时（15分钟）
)
...
```
data: {"choices": [{"delta": {"reasoning_content": "**Image-to-Video with Character Creation Begins**\n\nInitializing..."}}]}
data: {"choices": [{"delta": {"reasoning_content": "Uploading image..."}}]}
data: {"choices": [{"delta": {"reasoning_content": "**Video Generation Process Begins**\n\nGenerating video from image..."}}]}
data: {"choices": [{"delta": {"reasoning_content": "**Video Generation Completed**\n\nVideo URL: https://..."}}]}
...
### 关键点
- 传入 `image_url` + `text` + `create_character: true` = 图片生成视频 + 创建角色
- 流程：图片 → 生成视频 → 下载视频 → 创建角色 → 返回结果
- 响应中同时包含 **视频 URL** 和 **角色用户名**
- **角色会被保留并设为公开**，可用 `@username` 继续生成
- 建议设置较长超时（约 10-15 分钟），因为包含视频生成和角色创建两个步骤

### 与其他功能对比

| 功能 | 输入 | 参数 | 输出 |
|------|------|------|------|
| 功能三：视频创建角色+生成视频 | 视频 + prompt | `video_url` + `text` | 视频 + 角色 |
| 功能四：参考图生成视频 | 图片 + prompt | `image_url` + `text` | 仅视频 |
| **功能七：图片生成视频+创建角色** | 图片 + prompt | `image_url` + `text` + `create_character: true` | 视频 + 角色 |
...
class SoraClient:
    ...
    def generate_video_with_image_and_character(self, prompt: str, image_path: str,
                                                  character_description: str = None) -> tuple[str, str]:
        """使用参考图生成视频并创建角色，返回 (video_url, @username)"""
        with open(image_path, "rb") as f:
            image_base64 = base64.b64encode(f.read()).decode()
...
if __name__ == "__main__":
    ...
    # 4. 使用参考图生成视频并创建角色（新功能）
    video_url, username = client.generate_video_with_image_and_character(
        "人物开始微笑，转头看向镜头",
        "character_photo.jpg",
        character_description="一位面带微笑的人物"
    )
...
### Q: 只有图片没有视频，可以创建角色吗？
A: 可以。使用功能七（图片生成视频+创建角色），设置 `create_character: true`，系统会先用图片生成视频，再从生成的视频中提取角色。
